...
export function createScene(canvas) {
  ...
  controls.target.set(0, 1, 0); controls.enableDamping = true; controls.maxPolarAngle = 1.35; controls.minDistance = 4; controls.maxDistance = 110; controls.enablePan = false;
  ...
  const stars = new THREE.Points(sg, new THREE.PointsMaterial({ color: 0xffffff, size: .35, transparent: true, opacity: 0 })); scene.add(stars);

  // agua
  const wg = new THREE.PlaneGeometry(260, 260, 70, 70); wg.rotateX(-Math.PI / 2);
  ...
  // ilha (raio ~16: centro = vila do Bob, anel externo = cidade)
  const island = new THREE.Group(); scene.add(island);
  island.add(cyl(17.5, 20.0, 2.2, C.sand, 0, -1.9, 0, 15));
  island.add(cyl(16.0, 17.2, .5, C.grass, 0, -.2, 0, 15));
  const top = cyl(15.2, 15.9, .3, C.grass2, 0, .25, 0, 15); island.add(top);
  for (let i = 0; i < 14; i++) { const a = i * .45 + .3, r = 16.2 + Math.random() * 1.2; const rk = sphere(.25 + Math.random() * .35, C.rock, Math.cos(a) * r, .2, Math.sin(a) * r); rk.rotation.set(Math.random(), Math.random(), 0); island.add(rk); }
  island.add(palm(14.4, 6.8, 1), palm(-12.6, 9.2, 2.4), palm(-14.9, -4.9, 4), palm(11.9, -11.3, 5.5), palm(2.2, 15.1, .8), palm(-5.5, -14.5, 3), palm(9.5, 12.0, 2), palm(-15.5, 2.5, 1.5));
  // FLORESTA (madeira) — nordeste
  const floresta = new THREE.Group();
  const seed = [[5.5, -4.2, 1.1], [6.6, -5.4, .9], [4.6, -5.6, 1.3], [7.4, -3.4, 1.0], [5.8, -2.8, .8], [7.9, -5.2, 1.2], [4.0, -7.0, 1.0], [6.3, -6.9, .8], [8.3, -1.8, .9], [9.4, -6.6, 1.2], [8.8, -8.4, 1.0], [10.6, -4.4, .9], [6.0, -9.0, 1.1], [11.2, -7.6, 1.3], [4.2, -9.4, .9], [9.8, -9.9, .8], [12.4, -5.9, 1.0], [7.2, -11.0, 1.1]];
  for (const [x, z, s] of seed) floresta.add(tree(s, x, z));
  island.add(floresta); floresta.position.y = .55;
  // PEDREIRA (pedra) — oeste
  const pedreira = new THREE.Group();
  pedreira.add(cyl(3.0, 3.6, 1.6, 0x8d9099, -11.0, 0, .6, 7));
  pedreira.add(cyl(1.8, 2.3, 1.3, 0x9ca3af, -11.4, 1.6, .3, 6));
  pedreira.add(cyl(.9, 1.2, 1.0, 0xa8adb5, -11.8, 2.9, 0, 6));
  for (const [x, z, r] of [[-8.6, 2.8, .55], [-7.8, -1.2, .45], [-9.2, -2.4, .4], [-13.2, 3.0, .5], [-7.4, 1.6, .35], [-9.8, 4.2, .4], [-13.6, -1.8, .45]]) pedreira.add(sphere(r, C.rock, x, r * .6, z));
  island.add(pedreira); pedreira.position.y = .55;
  ...
  const aviao = buildAviao(); aviao.position.set(14.6, .2, -10.2); aviao.rotation.y = -.7; island.add(aviao);
  ...
  const unlock = { farol: farol(), vizinha: ilhaVizinha(), ponte: ponte(24), navio: navio(), montanha: montanha() };
  unlock.farol.position.set(-13.4, .55, -9.4); unlock.vizinha.position.set(33, 0, 4); unlock.ponte.position.set(17.0, 0, 4); unlock.navio.position.set(22, -.3, 18);
  unlock.montanha.position.set(0, -.5, -48);
  ...
  for (let i = 0; i < 8; i++) { const g = buildIlhaVizinhaNatural(i); const a = .6 + i * .8, r = 36 + (i % 2) * 7; g.position.set(Math.cos(a) * r, 0, Math.sin(a) * r); g.rotation.y = -a; scene.add(g); arquipelago.push(g); }
  window.__ilha = { scene, unlock, renderer, camera, controls, npcState, slots };
  let unlockState = {};

  const birds = new THREE.Group(); scene.add(birds);
  for (let i = 0; i < 4; i++) { const b = new THREE.Group(); const w1 = box(.35, .02, .1, 0x222222, -.17, 0, 0), w2 = box(.35, .02, .1, 0x222222, .17, 0, 0); w1.rotation.z = .5; w2.rotation.z = -.5; b.add(w1, w2); b.userData = { r: 18 + i * 1.8, s: .3 + i * .08, ph: i * 1.7, y: 8 + i * .7 }; birds.add(b); }

  scene.fog = new THREE.FogExp2(0x7fc4ff, .0045);
  let pulses = [];

  let lastDist = 0;
  function resize() { const w = canvas.clientWidth, h = canvas.clientHeight; if (canvas.width !== w * renderer.getPixelRatio() || canvas.height !== h * renderer.getPixelRatio()) { renderer.setSize(w, h, false); camera.aspect = w / h; camera.updateProjectionMatrix(); const want = camera.aspect < 1 ? 58 : 42; if (want !== lastDist) { const dir = camera.position.clone().sub(controls.target).normalize(); camera.position.copy(controls.target).add(dir.multiplyScalar(want)); lastDist = want; } } }
  ...
  function apply(v) {
    ...
    // distritos: um por habito, em lotes fixos
    const LOTES_D = [[6.2, .8, -Math.PI / 2 - .3], [-3.6, -5.6, .5], [-3.2, 5.6, -.6], [-8.0, -5.4, .9], [7.4, 6.8, -2.2], [-1.2, -10.2, .1]];
    const posTipo = {};
    for (const k of Object.keys(slots)) if (k.startsWith('dist_') && !(v.distritos || []).some(d => 'dist_' + d.id === k)) put(k, null, 0, 0);
    (v.distritos || []).forEach((d, i) => {
      const [x, z, r] = LOTES_D[i % LOTES_D.length];
      const b = d.tipo === 'academia' ? buildAcademia(d.lvl, d.prog, d.done) : d.tipo === 'capela' ? buildCapela(d.lvl, d.prog, d.done, night) : d.tipo === 'horta' ? buildHorta(d.lvl, d.prog, d.done) : buildGenerico(d.tipo, d.lvl, d.prog, d.done, night);
      put('dist_' + d.id, b, x, z, r); if (!posTipo[d.tipo]) posTipo[d.tipo] = [x, z + 2.2];
    });
    const dp = v.deposito || {};
    put('deposito', buildDeposito(dp.toras || 0, dp.pedras || 0, !!dp.madeiraHoje, !!dp.pedraHoje), 2.6, -2.8, .4);
    ...
    put('fogueira', buildFogueira(), 0, .6, 0);
    ...
    const temCasa = hab.some(h => h.id === 'casa');
    put('barraca', temCasa ? null : buildBarraca(), 1.4, 5.2, Math.PI + .3);
    put('casa', temCasa ? buildCasa(0xf1e7d0) : null, 1.4, 9.4, Math.PI);
    put('festa', hab.some(h => h.id === 'festa') ? buildBandeirinhas(14) : null, 0, .6, 0);
    // pontos de interesse (coordenadas da ilha) pras rotinas
    const P = { deposito: [2.6, -1.4], floresta: [7.0, -4.6], pedreira: [-8.2, 1.2], academia: posTipo.academia || [6.2, 3.2], horta: posTipo.horta || [-3.2, 3.4], capela: posTipo.capela || [-3.6, -3.2],
      casa: [1.4, 7.6], barraca: [1.4, 4.0], praca: [Math.cos(1.5) * 12.6, Math.sin(1.5) * 12.6], mercado: [Math.cos(1.35) * 10.0, Math.sin(1.35) * 10.0], porto: [Math.cos(1.5) * 15.2, Math.sin(1.5) * 15.2], fogueira: [0, .6] };
    const home = temCasa ? P.casa : P.barraca;
    const ROT = { bob: [P.deposito, P.floresta, P.deposito, P.pedreira, P.deposito, P.academia], amigo1: [P.praca, P.mercado, P.academia, P.deposito], amiga1: [P.horta, P.deposito, P.mercado],
      amor: [home, P.capela, P.horta, P.praca], bebe: [home, P.praca], amigo2: [P.mercado, P.porto, P.praca, P.academia], festa: [P.praca, P.capela, P.mercado] };
    ...
    for (const h of hab) {
      ...
      st.routine = ROT[h.id] || [P.praca, P.mercado]; st.home = home; st.tipo = h.tipo; st.speed = h.tipo === 'cachorro' ? 2.2 : h.tipo === 'crianca' ? 1.1 : 1.4;
    }
    ...
    put('moinho', tec.includes('moinho') ? buildMoinho() : null, -8.4, 8.2, .6);
    put('paineis', tec.includes('solar') ? buildPaineis() : null, 9.4, 3.6, 0);
    put('antena', tec.includes('radio') ? buildAntena(false) : null, -6.4, -9.6, 0);
    put('parabolica', tec.includes('internet') ? buildAntena(true) : null, 12.8, .4, 0);
    put('postes', tec.includes('gerador') ? buildPostes(night && v.done.devocional || night && tec.includes('solar')) : null, 0, 0, 0);
    // cidade: lotes fixos (angulo, raio); preenchidos na ordem da lista CIDADE
    const LOTES = [[1.05, 11.5], [1.35, 11.8], [1.65, 11.6], [1.95, 11.9], [2.25, 11.5], [.75, 12.2], [2.55, 12.0], [1.5, 14.6], [.45, 12.6], [2.85, 12.4]];
    const cid = v.cidade || [];
    cid.forEach((id, i) => { const [a, r] = LOTES[i % LOTES.length]; const rr = id === 'porto' ? 17.2 : r; put('cid_' + id, buildPredio(id), Math.cos(a) * rr, Math.sin(a) * rr, -a - Math.PI / 2); });
    for (const k of Object.keys(slots)) if (k.startsWith('cid_') && !cid.includes(k.slice(4))) put(k, null, 0, 0);
  ...
  function frame(now) {
    ...
    // fogueira
    const fg = slots.fogueira; if (fg) { const on = view && view.perfectToday && sunUp < .15; const fl = fg.getObjectByName('flame'), li = fg.getObjectByName('fire'); fl.visible = on; li.intensity = on ? 2.5 + Math.sin(t * 17) * .6 + Math.sin(t * 31) * .4 : 0; if (on) fl.scale.set(1 + Math.sin(t * 13) * .12, 1 + Math.sin(t * 9) * .18, 1); }
    // farol, moinho, fumaca do aviao
    if (unlockState.farol) unlock.farol.rotation.y = t * .8;
    if (slots.moinho) slots.moinho.getObjectByName('pas').rotation.z = t * (view && view.weather.clear ? 1.6 : .7);
